 ...
 **Input**:
    ...
    steadyStateFlag: {Bool}
        If true, the steady state problem is solved. If false, the transient. 

    diffeqFlag: {Bool}
        If the, alternative time stepping methods are from the package
...
grid, sol, nf,Y_grid = porousMedium(N_x, N_y, T_heat, 
	                         steadyState, 
...
begin#plotting q in x direction
	vis3=GridVisualizer(size=(600,600),xlabel="x",legend=:rt);vis3
